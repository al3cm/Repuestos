...
Public Class frmNota_Credito_Cliente
    ...
    Dim objNota_Credito_Cliente As New Nota_Credito_Cliente
    Dim objOrden_Venta As New Orden_Venta
    Dim objCliente As New Cliente
    Dim objKardex As New Kardex
    Dim objProducto_Almacen As New Producto_Almacen

    Private Sub frmNota_Credito_Cliente_FormClosing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing
        Limpiar()
    End Sub
    '---------------------------------------------
    '-----------------EVENTOS
    '---------------------------------------------
    Private Sub frmNota_Credito_Cliente_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            ' LÓGICA PERMISOS -------------------------------------------------
    ...
    Private Sub btnBuscar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnBuscar.Click
        If frmBuscar_Nota_Credito_Cliente.ShowDialog = Windows.Forms.DialogResult.OK Then
            Me.objNota_Credito_Cliente = frmBuscar_Nota_Credito_Cliente.objNota_Credito_Cliente
            Cargar()
            Habilitar(False)
            btnGrabar.Enabled = False
            btnModificar.Tag = "Modificar"
    ...
    Sub Grabar()
        Try
            If Valida() Then
                objNota_Credito_Cliente.fecha_emision = Me.dtpfecha_emision.Value
                objNota_Credito_Cliente.total = Me.TxtTotal.Text.Trim
                objNota_Credito_Cliente.subtotal = Me.txtPrecio_neto.Text.Trim
                objNota_Credito_Cliente.igv = Me.txtigv.Text.Trim
                objNota_Credito_Cliente.serie_nota_credito = Me.txtserie_documento.Text.ToString
                objNota_Credito_Cliente.nro_nota_credito = Me.txtnro_documento.Text.ToString
                objNota_Credito_Cliente.id_venta = Me.TxtIdOrden.Text.ToString
                objNota_Credito_Cliente.id_tipodocumento = Me.cbotipo_comprobante.SelectedValue
                objNota_Credito_Cliente.Saldo_Pendiente = Me.txtsaldo_pendiente.Text.ToString
                ...
                If MessageBox.Show("¿Desea registrar los datos de este Nota de Credito?", "Confirmación", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.Yes Then
                    objNota_Credito_Cliente = nNota_Credito_Cliente.RegistrarNota_Credito_Cliente(objNota_Credito_Cliente)
                    Dim Dgv As DataGridView
                    Dgv = Me.dtvListado_Productos
                    Try
                        For i As Integer = 0 To Dgv.RowCount - 1
                            objNota_Credito_Cliente.id_producto = CInt(Dgv.Item(0, i).Value)
                            objNota_Credito_Cliente.cantidad = CInt(Dgv.Item(2, i).Value)
                            objNota_Credito_Cliente.precio_venta = CDbl(Dgv.Item(4, i).Value)
                            objNota_Credito_Cliente.descuento = CDbl(Dgv.Item(5, i).Value)
                            objNota_Credito_Cliente.Sub_Total = CDbl(Dgv.Item(6, i).Value)
                            If objNota_Credito_Cliente.id_producto <> 0 Then
                                nNota_Credito_Cliente.RegistrarDetalle_Nota_Credito_Cliente(objNota_Credito_Cliente)
                                If Me.rbtDevolucion.Checked Then
                                    Dim Tabla As DataTable
                                    objKardex.id_producto = objNota_Credito_Cliente.id_producto
                                    objKardex.cantidad = objNota_Credito_Cliente.cantidad
                                    objKardex.precio = objNota_Credito_Cliente.precio_venta
                                    objKardex.Descuentro = objNota_Credito_Cliente.descuento
                                    objKardex.total = objNota_Credito_Cliente.Sub_Total
    ...
    Sub Modificar()
        Try
            ...
            objNota_Credito_Cliente.id_tipodocumento = Me.cbotipo_comprobante.SelectedValue
            objNota_Credito_Cliente.Saldo_Pendiente = Me.txtsaldo_pendiente.Text.ToString
            nNota_Credito_Cliente.ModificarNota_Credito_Cliente(objNota_Credito_Cliente)
            Dim Dgv As DataGridView
            Dgv = Me.dtvListado_Productos
            objNota_Credito_Cliente.id_tipodocumento = Me.cbotipo_comprobante.SelectedValue
            nNota_Credito_Cliente.EleminarDetalle_Nota_Credito_Cliente(objNota_Credito_Cliente.id_nota_credito)
            Try
                For i As Integer = 0 To Dgv.RowCount - 1
                    objNota_Credito_Cliente.id_producto = CInt(Dgv.Item(0, i).Value)
                    objNota_Credito_Cliente.cantidad = CInt(Dgv.Item(2, i).Value)
    ...
    Sub Eliminar()
        Try
            If MessageBox.Show("¿Desea eliminar los datos de este Orden de Venta?", "Confirmación", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.Yes Then
                nNota_Credito_Cliente.EleminarNota_Credito_Cliente(lblCodigo.Text)
                nNota_Credito_Cliente.EleminarDetalle_Nota_Credito_Cliente(lblCodigo.Text)
    ...
    Sub Cargar()
        Try
            ...
            Dim suma As Double
            Tabla = nNota_Credito_Cliente.Listar(objNota_Credito_Cliente.id_nota_credito)
            For Each Fila As DataRow In Tabla.Rows
                objNota_Credito_Cliente.fecha_emision = Fila.Item("fecha_emision")
                objNota_Credito_Cliente.total = Fila.Item("total")
                objNota_Credito_Cliente.subtotal = Fila.Item("subtotal")
                objNota_Credito_Cliente.igv = Fila.Item("igv")
                objNota_Credito_Cliente.nro_nota_credito = Fila.Item("nro_nota_credito")
                objNota_Credito_Cliente.serie_nota_credito = Fila.Item("serie_nota_credito")
                objNota_Credito_Cliente.id_nota_credito = Fila.Item("id_nota_credito")
                objNota_Credito_Cliente.Saldo_Pendiente = Fila.Item("saldo_pendiente")
            Next
            Me.lblCodigo.Text = objNota_Credito_Cliente.id_nota_credito
            Me.txtserie_documento.Text = objNota_Credito_Cliente.serie_nota_credito
            Me.txtnro_documento.Text = objNota_Credito_Cliente.nro_nota_credito
            Me.dtpfecha_emision.Value = objNota_Credito_Cliente.fecha_emision
            Me.txtPrecio_neto.Text = objNota_Credito_Cliente.subtotal
            Me.txtigv.Text = objNota_Credito_Cliente.igv
            Me.TxtTotal.Text = objNota_Credito_Cliente.total
            Me.txtsaldo_pendiente.Text = objNota_Credito_Cliente.Saldo_Pendiente
            Tabla = nNota_Credito_Cliente.ListarDetalle(objNota_Credito_Cliente.id_nota_credito)
            ToDescuento = 0.0
            suma = 0.0
            ...
            For Each Fila As DataRow In Tabla.Rows
                ...
                objNota_Credito_Cliente.id_venta = Fila.Item("id_venta")
                Me.dtvListado_Productos.Rows.Add(Id_Producto, Producto, Cantidad, Unidad, costo, Descuento, Total)
                ToDescuento = ToDescuento + Descuento
                suma = suma + Total
            Next
            Me.txtMonto_descuento.Text = ToDescuento
            Me.txtPrecio_bruto.Text = objNota_Credito_Cliente.total + ToDescuento
            If suma = objNota_Credito_Cliente.total Then
                Me.chkigv.Checked = True
            Else
                Me.chkigv.Checked = False
            End If
            Me.TxtIdOrden.Text = objNota_Credito_Cliente.id_venta
            Tabla = nOrden_Venta.Listar(objNota_Credito_Cliente.id_venta)
            For Each Fila As DataRow In Tabla.Rows
    ...
    Sub BuscarSerie()
        ...
        Dim serie As String
        id_Venta = nNota_Credito_Cliente.buscarid
