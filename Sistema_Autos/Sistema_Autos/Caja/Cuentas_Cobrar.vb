...
Public Class frmCuentas_Cobrar
    ...
    Private Sub frmCuentas_Cobrar_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            Cantidad = nPersonal.ContraAlmacen_Personal(id_Vededor)
            LlenaCombos()
            Limpiar()
            Me.txtfiltro_cliente.Focus()
    ...
    Private Sub btnSalir_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSalir.Click
        Me.Close()
    End Sub
    Private Sub txtfiltro_cliente_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtfiltro_cliente.TextChanged
        Try
            Me.DataGridView1.Rows.Clear()
            If Me.chbuscar_fechas.Checked Then
                Tabla = nLetras.ListarCuenta(Me.cboAlmacen.SelectedValue, Me.dtpfecha_desde.Value, Me.dtpfecha_hasta.Value, Me.txtfiltro_cliente.Text)
            Else
                Tabla = nLetras.ListarCuenta(Me.cboAlmacen.SelectedValue, Me.txtfiltro_cliente.Text)
            End If
            LlenaLista(Tabla)
        Catch ex As Exception
            'MessageBox.Show(ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
    ...
    Sub ActualizaLista()
        Try
            Me.DataGridView1.Rows.Clear()
            If Me.chbuscar_fechas.Checked Then
                Tabla = nLetras.ListarCuenta(Me.cboAlmacen.SelectedValue, Me.dtpfecha_desde.Value, Me.dtpfecha_hasta.Value)
            Else
                Tabla = nLetras.ListarCuenta(Me.cboAlmacen.SelectedValue)
            End If
            LlenaLista(Tabla)
        Catch ex As Exception
            MessageBox.Show(ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
    ...
    Sub LlenaLista(ByVal Datos As DataTable)
        ...
        For Each Fila As DataRow In Datos.Rows
            ...
            Dias = DateDiff(DateInterval.Day, fecha_emision, DateTime.Now())
            fecha_vencimiento = Fila.Item("fecha_vencimiento")
            vencimiento = DateDiff(DateInterval.Day, fecha_vencimiento, DateTime.Now())
            If vencimiento < 0 Then
                vencimiento = 0
            End If
            monto = Fila.Item("monto")
            Moneda = Fila.Item("Moneda")
            nombres = Fila.Item("nombres")
            ap_paterno = Fila.Item("ap_paterno")
            ap_materno = Fila.Item("ap_materno")
            Vededor = nombres & " " & ap_paterno & " " & ap_materno
            direccion = Fila.Item("direccion")
            Me.DataGridView1.Rows.Add(razon_social, coprobrante, fecha_emision, Dias, fecha_vencimiento, monto, Moneda, Vededor, direccion)
        Next
    End Sub
